...
ZN4XLaunchEnvironment.ZN4XAutoFarmReferenceWidth = tonumber(ZN4XLaunchEnvironment.ZN4XAutoFarmReferenceWidth)
	or (workspace.CurrentCamera and workspace.CurrentCamera.ViewportSize.X)
	or 1366
ZN4XLaunchEnvironment.ZN4XAutoFarmReferenceHeight = tonumber(ZN4XLaunchEnvironment.ZN4XAutoFarmReferenceHeight)
	or (workspace.CurrentCamera and workspace.CurrentCamera.ViewportSize.Y)
	or 768
local ZN4XPendingAutoFarm = ZN4XLaunchEnvironment.ZN4XPendingAutoFarm == true
ZN4XLaunchEnvironment.ZN4XPendingAutoFarm = nil
...
if not modoteste then
	...
	if not validSession then
		pcall(function()
			player:Kick("Voce e idiota?.")
		end)
		return
	end
	if tostring(session.Server) ~= "AirsoftFE" then
		pcall(function()
			player:Kick("Este menu pertence ao Airsoft FE.")
		end)
		return
	end
...
-- Este arquivo pertence exclusivamente ao Airsoft FE.
ZN4XExploitSelectedList = "M"
ZN4XSkipBoot = true
...
ZN4XCurrentProfileName = modoteste and "" or tostring(ZN4XLaunchEnvironment.ZN4XAccessProfileName or "ZN4X MENU")
ZN4XCurrentProfileRole = modoteste and "" or tostring(ZN4XLaunchEnvironment.ZN4XAccessProfileRole or "Free")
ZN4XCurrentProfileImage = modoteste and "" or tostring(ZN4XLaunchEnvironment.ZN4XAccessProfileImage or ZN4XFreeProfileImage)
...
local aimbotExcludeDeads = true
ZN4XAirsoftAutoTeam = ZN4XAirsoftAutoTeam ~= false
...
ZN4XAirsoftHiddenPosition = Vector3.new(-272, 291, -2028)
ZN4XAirsoftKillBusy = false
ZN4XAirsoftKillRequestId = 0
ZN4XAirsoftKillAllLoopEnabled = false
ZN4XAirsoftKillAllLoopToken = 0
ZN4XAirsoftKillAllRunning = false
ZN4XAirsoftKillAllRunToken = 0
ZN4XAirsoftKillAllBypassState = "idle"
ZN4XAirsoftAutoFarmEnabled = false
ZN4XAirsoftAutoFarmToken = 0
ZN4XAirsoftAutoFarmOwnsInvisibility = false
ZN4XAirsoftAutoFarmCursorForced = false
ZN4XAirsoftKillMode = "V2"

function ZN4XAirsoftSameTeam(targetPlayer)
	...
	if player.Neutral == false and targetPlayer.Neutral == false then
		return player.TeamColor == targetPlayer.TeamColor
	end
	return false
end

function ZN4XAirsoftHasSpawnProtection(character)
	if not character then return false end
	for _, object in ipairs(character:GetDescendants()) do
		if object:IsA("ForceField") then return true end
	end
	return false
end
...
local selectedPlayerStrongBlue = Color3.fromRGB(0, 110, 255)
...
local function notify(_, text)
	...
	return function(newText)
		if messageLabel.Parent then messageLabel.Text = tostring(newText or "") end
	end
end
...
local function isFakeSessionMode(mode)
	return mode == "Solo Session" or mode == "Semi Solo Session"
end

local function isSoloSessionActive()
...
local function getBestAimbotTarget()
	...
	for _, otherPlayer in ipairs(Players:GetPlayers()) do
		if otherPlayer ~= player and not aimbotFriends[otherPlayer.UserId] then
			...
			if not (ZN4XAirsoftAutoTeam and ZN4XAirsoftSameTeam(otherPlayer))
				and not (teamName and aimbotIgnoredTeams[teamName]) then
		...
		if otherPlayer ~= player then
			...
			if humanoid and humanoid.Health > 0 and rootPart then
				...
				if rootOnScreen and distance <= distanceLimit then
					...
					local roleColor = isSelected and selectedPlayerStrongBlue or (ZN4XAirsoftSameTeam(otherPlayer)
						and Color3.fromRGB(80, 235, 120)
						or Color3.fromRGB(235, 75, 95))

					entry.boxStroke.Color = roleColor or colors.accent
					entry.boxStroke.Thickness = isSelected and 3 or 1.5
					...
					entry.line.Size = UDim2.fromOffset(math.max(delta.Magnitude, 1), isSelected and 4 or 2)
	...
	if enabled then
		...
		if isFakeSessionMode(invisibilityMode) then
			if invisibilityMode == "Semi Solo Session" then
				...
				local hiddenPosition = ZN4XAirsoftHiddenPosition
...
ZN4XAirsoftAutoFarmBanner = Instance.new("Frame")
ZN4XAirsoftAutoFarmBanner.Name = "AutoFarmBanner"
ZN4XAirsoftAutoFarmBanner.AnchorPoint = Vector2.new(0.5, 1)
ZN4XAirsoftAutoFarmBanner.Position = UDim2.new(0.5, 0, 1, -7)
ZN4XAirsoftAutoFarmBanner.Size = UDim2.fromOffset(390, 40)
ZN4XAirsoftAutoFarmBanner.BackgroundColor3 = Color3.fromRGB(8, 10, 15)
ZN4XAirsoftAutoFarmBanner.BackgroundTransparency = 0.02
ZN4XAirsoftAutoFarmBanner.BorderSizePixel = 0
ZN4XAirsoftAutoFarmBanner.Visible = ZN4XAirsoftAutoFarmEnabled
ZN4XAirsoftAutoFarmBanner.ZIndex = 3100
ZN4XAirsoftAutoFarmBanner.Parent = gui
makeCorner(ZN4XAirsoftAutoFarmBanner, 3)
makeStroke(ZN4XAirsoftAutoFarmBanner, Color3.fromRGB(40, 45, 58), 0.25, 1)

ZN4XAirsoftAutoFarmBannerAccent = Instance.new("Frame")
ZN4XAirsoftAutoFarmBannerAccent.Name = "Accent"
ZN4XAirsoftAutoFarmBannerAccent.Position = UDim2.fromOffset(0, 6)
ZN4XAirsoftAutoFarmBannerAccent.Size = UDim2.fromOffset(3, 28)
ZN4XAirsoftAutoFarmBannerAccent.BackgroundColor3 = colors.accent
ZN4XAirsoftAutoFarmBannerAccent.BorderSizePixel = 0
ZN4XAirsoftAutoFarmBannerAccent.ZIndex = 3101
ZN4XAirsoftAutoFarmBannerAccent.Parent = ZN4XAirsoftAutoFarmBanner
makeCorner(ZN4XAirsoftAutoFarmBannerAccent, 2)

ZN4XAirsoftAutoFarmBannerEndAccent = Instance.new("Frame")
ZN4XAirsoftAutoFarmBannerEndAccent.Name = "EndAccent"
ZN4XAirsoftAutoFarmBannerEndAccent.AnchorPoint = Vector2.new(1, 0)
ZN4XAirsoftAutoFarmBannerEndAccent.Position = UDim2.new(1, 0, 0, 6)
ZN4XAirsoftAutoFarmBannerEndAccent.Size = UDim2.fromOffset(3, 28)
ZN4XAirsoftAutoFarmBannerEndAccent.BackgroundColor3 = colors.accent
ZN4XAirsoftAutoFarmBannerEndAccent.BorderSizePixel = 0
ZN4XAirsoftAutoFarmBannerEndAccent.ZIndex = 3101
ZN4XAirsoftAutoFarmBannerEndAccent.Parent = ZN4XAirsoftAutoFarmBanner
makeCorner(ZN4XAirsoftAutoFarmBannerEndAccent, 2)

ZN4XAirsoftAutoFarmBannerText = makeText(
	ZN4XAirsoftAutoFarmBanner,
	"Voce Esta com Autofarm ligado aperte F3 para desativar",
	13,
	Color3.fromRGB(230, 234, 244),
	Enum.Font.GothamMedium
)
ZN4XAirsoftAutoFarmBannerText.Position = UDim2.fromOffset(14, 0)
ZN4XAirsoftAutoFarmBannerText.Size = UDim2.new(1, -28, 1, 0)
ZN4XAirsoftAutoFarmBannerText.TextXAlignment = Enum.TextXAlignment.Left
ZN4XAirsoftAutoFarmBannerText.ZIndex = 3101

ZN4XAirsoftAutoFarmMouseReleaseWindow = Instance.new("TextButton")
disableAutoLocalize(ZN4XAirsoftAutoFarmMouseReleaseWindow)
ZN4XAirsoftAutoFarmMouseReleaseWindow.Name = "AutoFarmMouseReleaseWindow"
ZN4XAirsoftAutoFarmMouseReleaseWindow.Position = UDim2.fromOffset(5, 5)
ZN4XAirsoftAutoFarmMouseReleaseWindow.Size = UDim2.fromOffset(14, 14)
ZN4XAirsoftAutoFarmMouseReleaseWindow.BackgroundColor3 = Color3.fromRGB(8, 10, 15)
ZN4XAirsoftAutoFarmMouseReleaseWindow.BackgroundTransparency = 0.15
ZN4XAirsoftAutoFarmMouseReleaseWindow.BorderSizePixel = 0
ZN4XAirsoftAutoFarmMouseReleaseWindow.Text = ""
ZN4XAirsoftAutoFarmMouseReleaseWindow.AutoButtonColor = false
ZN4XAirsoftAutoFarmMouseReleaseWindow.Active = true
ZN4XAirsoftAutoFarmMouseReleaseWindow.Modal = true
ZN4XAirsoftAutoFarmMouseReleaseWindow.Selectable = false
ZN4XAirsoftAutoFarmMouseReleaseWindow.Visible = false
ZN4XAirsoftAutoFarmMouseReleaseWindow.ZIndex = 3200
ZN4XAirsoftAutoFarmMouseReleaseWindow.Parent = gui
makeCorner(ZN4XAirsoftAutoFarmMouseReleaseWindow, 3)
makeStroke(ZN4XAirsoftAutoFarmMouseReleaseWindow, colors.accent, 0.1, 1)

function ZN4XAirsoftUpdateAutoFarmBanner()
	if ZN4XAirsoftAutoFarmBanner and ZN4XAirsoftAutoFarmBanner.Parent then
		ZN4XAirsoftAutoFarmBanner.Visible = ZN4XAirsoftAutoFarmEnabled == true
	end
end
...
local function makeSectionPanel(parent, titleText, position, size)
...
local function makeScrollableSectionPanel(parent, titleText, position, size)
	local panel = Instance.new("Frame")
	panel.Name = titleText .. "Panel"
	panel.Position = position
	panel.Size = size
...
local function makeSwitch(parent, labelText, isOn, onChanged)
...
local function makeKeyBindInput(parent, labelText, getKeyCode, onChanged)
	local row = Instance.new("Frame")
	row.Name = labelText:gsub("%s+", "") .. "Row"
	row.BackgroundTransparency = 1
...
local function makeOptionalKeyBindInput(parent, labelText)
	local row = Instance.new("Frame")
	row.Name = labelText:gsub("%s+", "") .. "Row"
	row.BackgroundTransparency = 1
	row.Size = UDim2.new(1, 0, 0, 30)
...
local function makeColorSwatch(parent, labelText)
	local row = Instance.new("Frame")
	row.Name = labelText:gsub("%s+", "") .. "Row"
	row.BackgroundTransparency = 1
	row.Size = UDim2.new(1, 0, 0, 28)
...
local function makeModeSelector(parent, labelText)
...
local function makeHeightSelector(parent, labelText)
	local row = Instance.new("Frame")
	row.Name = labelText:gsub("%s+", "") .. "Row"
	row.BackgroundTransparency = 1
...
local function makeCompactSelector(parent, labelText, values, getValue, onChanged, buttonWidth)
	local row = Instance.new("Frame")
...
local function makeSlider(parent, labelText, getValue, minValue, maxValue, onChanged)
	local row = Instance.new("Frame")
	row.Name = labelText:gsub("%s+", "") .. "Row"
	row.BackgroundTransparency = 1
...
local function makeFunctionSeparator(parent)
...
local function makeSubSectionTitle(parent, titleText)
	local row = Instance.new("Frame")
...
local function makeJogadorPage()
	...
	if ZN4XExploitSelectedList == "M" then
		if forceThirdPersonEnabled then
			setForceThirdPerson(false)
		end
		...
		if flyMode == "Semi Solo Session" then
			if flyEnabled then
				setFly(false)
			end
			flyMode = "Solo Session"
		end
	end
	...
	if ZN4XExploitSelectedList ~= "M" then
		makeSwitch(localBody, "Forcar 3 Pessoa", forceThirdPersonEnabled, function(enabled)
			setForceThirdPerson(enabled)
		end)
	...
	if ZN4XExploitSelectedList ~= "M" then
		makeSwitch(localBody, "Desbloquear Trava No Shift", shiftLockEnabled, function(enabled)
...
local function makeMiraPage()
	...
	makeSwitch(aimbotBody, "Auto Time", ZN4XAirsoftAutoTeam, function(enabled)
		ZN4XAirsoftAutoTeam = enabled
	end)
...
local function makeValueRow(parent, labelText, valueText)
	local row = Instance.new("Frame")
	row.Name = labelText:gsub("%s+", "") .. "Row"
	row.BackgroundTransparency = 1
	row.Size = UDim2.new(1, 0, 0, 28)
...
local function makeVisuaisPage()
...
function ZN4XAirsoftFireWeapon(camera, pressed)
	pcall(function()
		if pressed then
			if mouse1press then
				mouse1press()
	...
	pcall(function()
		...
		local viewport = camera.ViewportSize
		virtualInput:SendMouseButtonEvent(viewport.X / 2, viewport.Y / 2, 0, pressed, game, 0)
	end)
end

function ZN4XAirsoftKillTarget(targetPlayer, requestId, silent, killMode, duration)
	if not targetPlayer or targetPlayer == player then return false end
	if ZN4XAirsoftSameTeam(targetPlayer) then
		if not silent then notify("ZN4X", "Player do seu time") end
		return false
	end
	...
	local targetHumanoid = targetCharacter and targetCharacter:FindFirstChildOfClass("Humanoid")
	local targetRoot = targetCharacter and targetCharacter:FindFirstChild("HumanoidRootPart")
	local targetTorso = targetCharacter and (targetCharacter:FindFirstChild("UpperTorso") or targetCharacter:FindFirstChild("Torso") or targetRoot)
	local camera = workspace.CurrentCamera
	if not humanoid or not rootPart or not targetHumanoid or targetHumanoid.Health <= 0 or not targetRoot or not targetTorso or not camera then
	...
	local deadline = os.clock() + (tonumber(duration) or 5)
	local nextShot = 0
	local releaseShotAt = 0
	local triggerPressed = false

	local actionOk, actionFailure = pcall(function()
		while gui.Parent
			and not gui:GetAttribute("Cleaning")
			and requestId == ZN4XAirsoftKillRequestId
			and targetPlayer.Parent
			and targetHumanoid.Parent
			and targetHumanoid.Health > 0
			and targetRoot.Parent
			and os.clock() < deadline do
			local behind = targetRoot.CFrame * CFrame.new(0, 0.4, 3.2)
			if killMode == "V2" then
				rootPart.CFrame = returnCFrame
			else
				rootPart.CFrame = CFrame.new(behind.Position, targetTorso.Position)
			end
			rootPart.AssemblyLinearVelocity = Vector3.new(0, 0, 0)
			rootPart.AssemblyAngularVelocity = Vector3.new(0, 0, 0)
			local attackCameraPosition = killMode == "V2"
				and (behind.Position + Vector3.new(0, 1.5, 0))
				or (head and head.Position or rootPart.Position + Vector3.new(0, 1.5, 0))
			camera.CameraType = Enum.CameraType.Scriptable
			camera.CFrame = CFrame.new(attackCameraPosition, targetTorso.Position)
			if triggerPressed and os.clock() >= releaseShotAt then
				ZN4XAirsoftFireWeapon(camera, false)
				triggerPressed = false
			end
			if os.clock() >= nextShot then
				nextShot = os.clock() + 0.12
				releaseShotAt = os.clock() + 0.055
				ZN4XAirsoftFireWeapon(camera, true)
				triggerPressed = true
			end
			RunService.RenderStepped:Wait()
		end
	...
	if not actionOk then error(actionFailure, 0) end
	return targetHumanoid.Health <= 0
end

function ZN4XAirsoftStartKill(targetPlayer, silent, duration)
	if ZN4XAirsoftKillBusy then
		return false
	end
	ZN4XAirsoftKillBusy = true
	ZN4XAirsoftKillRequestId = ZN4XAirsoftKillRequestId + 1
	local requestId = ZN4XAirsoftKillRequestId
	if not silent then
		menuOpen = false
		root.Visible = false
		updateCustomCursor()
	...
	task.spawn(function()
		local success, failure = pcall(function()
			ZN4XAirsoftKillTarget(targetPlayer, requestId, silent, ZN4XAirsoftKillMode, duration)
		end)
		ZN4XAirsoftKillBusy = false
		if not success then warn("ZN4X matar player: " .. tostring(failure)) end
	end)
	return true
end

function ZN4XAirsoftKillAllOnce(silent)
	if ZN4XAirsoftKillAllLoopEnabled then
		ZN4XAirsoftKillAllLoopEnabled = false
		ZN4XAirsoftKillAllLoopToken = ZN4XAirsoftKillAllLoopToken + 1
		ZN4XAirsoftKillRequestId = ZN4XAirsoftKillRequestId + 1
	end
	ZN4XAirsoftKillAllRunToken = ZN4XAirsoftKillAllRunToken + 1
	local runToken = ZN4XAirsoftKillAllRunToken
	ZN4XAirsoftKillAllRunning = true
	if not silent then
		menuOpen = false
		root.Visible = false
		updateCustomCursor()
	...
	task.spawn(function()
		...
		until not ZN4XAirsoftKillBusy or runToken ~= ZN4XAirsoftKillAllRunToken

		local targets = {}
		for _, targetPlayer in ipairs(Players:GetPlayers()) do
			if targetPlayer ~= player and not ZN4XAirsoftSameTeam(targetPlayer) then
				table.insert(targets, targetPlayer)
			end
		end

		for _, targetPlayer in ipairs(targets) do
			if runToken ~= ZN4XAirsoftKillAllRunToken or not gui.Parent or gui:GetAttribute("Cleaning") then break end
			local protectionDeadline = os.clock() + 4
			while targetPlayer.Parent and ZN4XAirsoftHasSpawnProtection(targetPlayer.Character)
				and os.clock() < protectionDeadline and runToken == ZN4XAirsoftKillAllRunToken do
				task.wait(0.1)
			end
			...
			local targetHumanoid = targetCharacter and targetCharacter:FindFirstChildOfClass("Humanoid")
			local targetRoot = targetCharacter and targetCharacter:FindFirstChild("HumanoidRootPart")
			if targetPlayer.Parent and not ZN4XAirsoftSameTeam(targetPlayer)
				and targetHumanoid and targetHumanoid.Health > 0 and targetRoot
				and not ZN4XAirsoftHasSpawnProtection(targetCharacter) then
				local started = ZN4XAirsoftStartKill(targetPlayer, true, 5)
				if started then
					repeat task.wait(0.03)
					until not ZN4XAirsoftKillBusy or runToken ~= ZN4XAirsoftKillAllRunToken
				end
		...
		if runToken == ZN4XAirsoftKillAllRunToken then
			ZN4XAirsoftKillAllRunning = false
		end
	end)
	return true
...
function ZN4XAirsoftStartKillAllWithBypass()
	if ZN4XAirsoftKillAllBypassState == "loading" then return false end
	if ZN4XAirsoftKillAllBypassState == "loaded" then
		return ZN4XAirsoftKillAllOnce(false)
	end

	ZN4XAirsoftKillAllBypassState = "loading"
	local updateNotification = notify("ZN4X", "Carregando Bypass")
	task.delay(1, function()
		if ZN4XAirsoftKillAllBypassState ~= "loading" then return end
		ZN4XAirsoftKillAllBypassState = "loaded"
		if type(updateNotification) == "function" then updateNotification("Bypass Carregado") end
		if gui.Parent and not gui:GetAttribute("Cleaning") then
			ZN4XAirsoftKillAllOnce(false)
		end
	end)
	return true
end

function ZN4XAirsoftSetKillAllLoop(enabled)
	local wasEnabled = ZN4XAirsoftKillAllLoopEnabled == true
	ZN4XAirsoftKillAllLoopEnabled = enabled == true
	ZN4XAirsoftKillAllLoopToken = ZN4XAirsoftKillAllLoopToken + 1
	local loopToken = ZN4XAirsoftKillAllLoopToken
	if not ZN4XAirsoftKillAllLoopEnabled then
		if wasEnabled then ZN4XAirsoftKillRequestId = ZN4XAirsoftKillRequestId + 1 end
		return true
	end
	ZN4XAirsoftKillAllRunToken = ZN4XAirsoftKillAllRunToken + 1
	ZN4XAirsoftKillAllRunning = false
	if ZN4XAirsoftKillBusy then ZN4XAirsoftKillRequestId = ZN4XAirsoftKillRequestId + 1 end

	task.spawn(function()
		local respawnState = {}
		while ZN4XAirsoftKillAllLoopEnabled
			and loopToken == ZN4XAirsoftKillAllLoopToken
			and gui.Parent
			and not gui:GetAttribute("Cleaning") do
			local attacked = false
			for _, targetPlayer in ipairs(Players:GetPlayers()) do
				if not ZN4XAirsoftKillAllLoopEnabled or loopToken ~= ZN4XAirsoftKillAllLoopToken then break end
				if targetPlayer ~= player and not ZN4XAirsoftSameTeam(targetPlayer) then
					local character = targetPlayer.Character
					local humanoid = character and character:FindFirstChildOfClass("Humanoid")
					local state = respawnState[targetPlayer.UserId]
					if not state or state.character ~= character then
						local isRespawn = state ~= nil
						state = {
							character = character,
							readyAt = (isRespawn or ZN4XAirsoftHasSpawnProtection(character)) and (os.clock() + 4) or 0,
						}
						respawnState[targetPlayer.UserId] = state
					end
					local ready = humanoid and humanoid.Health > 0
						and os.clock() >= state.readyAt
						and not ZN4XAirsoftHasSpawnProtection(character)
					if ready and not ZN4XAirsoftKillBusy then
						ZN4XAirsoftStartKill(targetPlayer, true, 5)
						attacked = true
						repeat task.wait(0.03)
						until not ZN4XAirsoftKillBusy
							or not ZN4XAirsoftKillAllLoopEnabled
							or loopToken ~= ZN4XAirsoftKillAllLoopToken
					end
				end
			end
			task.wait(attacked and 0.08 or 0.25)
		end
	end)
	return true
end

function ZN4XAirsoftSetAutoFarmCursorForced(enabled)
	ZN4XAirsoftAutoFarmCursorForced = enabled == true
	if ZN4XAirsoftAutoFarmMouseReleaseWindow and ZN4XAirsoftAutoFarmMouseReleaseWindow.Parent then
		ZN4XAirsoftAutoFarmMouseReleaseWindow.Visible = ZN4XAirsoftAutoFarmCursorForced
	end
	pcall(function()
		RunService:UnbindFromRenderStep("ZN4X_AutoFarmFreeMouse")
	end)

	if not ZN4XAirsoftAutoFarmCursorForced then
		pcall(function()
			UserInputService.MouseBehavior = Enum.MouseBehavior.Default
			UserInputService.MouseIconEnabled = true
	...
	local function unlockMouse()
		if not ZN4XAirsoftAutoFarmCursorForced then return end
		pcall(function()
			UserInputService.MouseBehavior = Enum.MouseBehavior.Default
			UserInputService.MouseIconEnabled = true
...
function ZN4XAirsoftActivateGuiButtonAt(x, y)
	local objects = {}
	pcall(function()
		objects = playerGui:GetGuiObjectsAtPosition(x, y)
	end)

	for _, object in ipairs(objects) do
		if object:IsA("GuiButton") and not object:IsDescendantOf(gui) then
...
function ZN4XAirsoftGetAutoFarmClickPosition()
	local camera = workspace.CurrentCamera
	local viewport = camera and camera.ViewportSize or Vector2.new(1366, 768)
	local referenceWidth = tonumber(ZN4XLaunchEnvironment.ZN4XAutoFarmReferenceWidth) or 1366
	local referenceHeight = tonumber(ZN4XLaunchEnvironment.ZN4XAutoFarmReferenceHeight) or 768
	local x = math.floor((301 / referenceWidth) * viewport.X + 0.5)
	local y = math.floor((707 / referenceHeight) * viewport.Y + 0.5)
	return math.clamp(x, 0, math.max(0, viewport.X - 1)), math.clamp(y, 0, math.max(0, viewport.Y - 1))
end

function ZN4XAirsoftAutoFarmClickRespawn()
	menuOpen = false
	root.Visible = false
	updateCustomCursor()
	ZN4XAirsoftSetAutoFarmCursorForced(true)
	local virtualInput = game:GetService("VirtualInputManager")
	local targetX, targetY = ZN4XAirsoftGetAutoFarmClickPosition()
	for _ = 1, 3 do
		forceFreeMouse()
		pcall(function()
			virtualInput:SendMouseMoveEvent(targetX, targetY, game)
		end)
		RunService.RenderStepped:Wait()
	end
	task.wait(0.03)
	...
	task.wait(0.05)
	ZN4XAirsoftActivateGuiButtonAt(targetX, targetY)
end

function ZN4XAirsoftAutoFarmClickCurrentPosition()
	local clickedWithExecutor = false
	pcall(function()
		if type(mouse1click) == "function" then
			mouse1click()
			clickedWithExecutor = true
		end
	end)
	if clickedWithExecutor then return end
	pcall(function()
		local virtualInput = game:GetService("VirtualInputManager")
...
function ZN4XAirsoftSetAutoFarm(enabled)
	ZN4XAirsoftAutoFarmEnabled = enabled == true
	ZN4XAirsoftAutoFarmToken = ZN4XAirsoftAutoFarmToken + 1
	ZN4XAirsoftUpdateAutoFarmBanner()
	local autoFarmToken = ZN4XAirsoftAutoFarmToken
	if not ZN4XAirsoftAutoFarmEnabled then
		ZN4XAirsoftSetAutoFarmCursorForced(false)
		ZN4XAirsoftSetKillAllLoop(false)
		local shouldDisableInvisibility = ZN4XAirsoftAutoFarmOwnsInvisibility and invisibilityEnabled
		ZN4XAirsoftAutoFarmOwnsInvisibility = false
		if shouldDisableInvisibility then
			task.spawn(function()
				repeat task.wait(0.03)
				until not ZN4XAirsoftKillBusy or autoFarmToken ~= ZN4XAirsoftAutoFarmToken
				if not ZN4XAirsoftAutoFarmEnabled and autoFarmToken == ZN4XAirsoftAutoFarmToken then
					setInvisibility(false)
				end
			end)
		end
	...
	if not invisibilityEnabled then
		invisibilityMode = "Solo Session"
		setInvisibility(true)
		ZN4XAirsoftAutoFarmOwnsInvisibility = invisibilityEnabled == true
	end
	ZN4XAirsoftSetKillAllLoop(true)
	menuOpen = false
	root.Visible = false
	updateCustomCursor()
	notify("ZN4X Auto Farm", 'Deixe o Roblox em tela cheia "1920x1080"')

	task.spawn(function()
		local missingDistanceSince
		local nextRespawnClick = 0
		local nextPeriodicClick = os.clock() + 300
		local nextSuicide = os.clock() + 180
		local lowPopulationSince
		local serverHopRequested = false
		while ZN4XAirsoftAutoFarmEnabled
			and autoFarmToken == ZN4XAirsoftAutoFarmToken
			and gui.Parent
			and not gui:GetAttribute("Cleaning") do
			local now = os.clock()
			if now >= nextPeriodicClick then
				nextPeriodicClick = now + 300
				ZN4XAirsoftAutoFarmClickCurrentPosition()
			end
			if now >= nextSuicide then
				local humanoid = getHumanoid()
			...
			for _, otherPlayer in ipairs(Players:GetPlayers()) do
				if otherPlayer ~= player and not ZN4XAirsoftSameTeam(otherPlayer) then
					enemyPlayers = enemyPlayers + 1
				end
			end
			local lowPopulation = totalPlayers <= 5 or enemyPlayers <= 3
			if lowPopulation then
				lowPopulationSince = lowPopulationSince or now
			elseif not serverHopRequested then
				lowPopulationSince = nil
			end
			if not serverHopRequested and lowPopulationSince and now - lowPopulationSince >= 300 then
				local enterMax = ZN4XLaunchEnvironment.zn4xentermax
				if type(enterMax) == "function" then
					serverHopRequested = true
					ZN4XAirsoftSetKillAllLoop(false)
					local ok, entered, enterError = pcall(enterMax, { autoFarm = true, server = "AirsoftFE" })
					if not ok or entered ~= true then
						serverHopRequested = false
						lowPopulationSince = now
						notify("ZN4X Auto Farm", "Falha ao trocar servidor: " .. tostring(enterError or entered))
					end
				else
					lowPopulationSince = now
				end
			...
			local distanceAvailable = getPlayerListDistance(player) < math.huge
			if not distanceAvailable then
				if ZN4XAirsoftKillAllLoopEnabled then ZN4XAirsoftSetKillAllLoop(false) end
				missingDistanceSince = missingDistanceSince or now
				if now - missingDistanceSince >= 20 then
					if not ZN4XAirsoftAutoFarmCursorForced then ZN4XAirsoftSetAutoFarmCursorForced(true) end
					if now >= nextRespawnClick and not ZN4XAirsoftKillBusy then
						nextRespawnClick = now + 2
						ZN4XAirsoftAutoFarmClickRespawn()
					end
				end
			else
				if ZN4XAirsoftAutoFarmCursorForced then ZN4XAirsoftSetAutoFarmCursorForced(false) end
				missingDistanceSince = nil
				nextRespawnClick = 0
				if not invisibilityEnabled then
					invisibilityMode = "Solo Session"
					setInvisibility(true)
					if invisibilityEnabled then ZN4XAirsoftAutoFarmOwnsInvisibility = true end
				end
				if not serverHopRequested and not ZN4XAirsoftKillAllLoopEnabled then ZN4XAirsoftSetKillAllLoop(true) end
			end
			task.wait(0.15)
		end
	end)
...
function makeExploitPage()
	...
	local function exploitNotice(text)
		notify("ZN4X Exploit", text)
	end

	makeActionButton(exploitsBody, "Matar Todos", function()
		ZN4XAirsoftStartKillAllWithBypass()
	end)

	makeSwitch(exploitsBody, "Matar Todos Loop", ZN4XAirsoftKillAllLoopEnabled, function(enabled)
		return ZN4XAirsoftSetKillAllLoop(enabled)
	end)

	makeSwitch(extrasBody, "Auto Farm", ZN4XAirsoftAutoFarmEnabled, function(enabled)
		return ZN4XAirsoftSetAutoFarm(enabled)
	end)
	...
	local serverListRequestId = 0
	local refreshServersButton

	local function clearServerRows()
		for _, child in ipairs(othersBody:GetChildren()) do
			if child:GetAttribute("ZN4XServerListItem") then child:Destroy() end
		end
	end

	local function makeServerStatus(text)
		local row = Instance.new("Frame")
		row.Name = "ServerStatus"
		row.Size = UDim2.new(1, 0, 0, 32)
		row.BackgroundColor3 = Color3.fromRGB(16, 19, 28)
		row.BorderSizePixel = 0
		row:SetAttribute("ZN4XServerListItem", true)
		row.Parent = othersBody
		makeCorner(row, 5)
		makeStroke(row, colors.line, 0.35, 1)
		local label = makeText(row, text, 12, colors.muted, Enum.Font.GothamMedium)
		label.Position = UDim2.fromOffset(10, 0)
		label.Size = UDim2.new(1, -20, 1, 0)
	end

	local function makeServerRow(server, index)
		local playing = tonumber(server.playing) or 0
		local maxPlayers = tonumber(server.maxPlayers) or 0
		local ping = math.floor((tonumber(server.ping) or 0) + 0.5)
		local fps = math.floor((tonumber(server.fps) or 0) + 0.5)
		local jobId = tostring(server.id or "")
		local shortId = jobId ~= "" and jobId:sub(1, 8) or "N/A"

		local row = Instance.new("Frame")
		row.Name = "Server" .. tostring(index)
		...
		local title = makeText(row, string.format("Server %d - %d/%d", index, playing, maxPlayers), 12, colors.text, Enum.Font.GothamSemibold)
		title.Position = UDim2.fromOffset(10, 4)
		title.Size = UDim2.new(1, -92, 0, 20)

		local details = makeText(row, string.format("Ping: %d ms | FPS: %d | ID: %s", ping, fps, shortId), 10, colors.muted, Enum.Font.GothamMedium)
		details.Position = UDim2.fromOffset(10, 25)
		details.Size = UDim2.new(1, -92, 0, 18)

		local enterButton = Instance.new("TextButton")
		disableAutoLocalize(enterButton)
		enterButton.AnchorPoint = Vector2.new(1, 0.5)
		enterButton.Position = UDim2.new(1, -7, 0.5, 0)
		enterButton.Size = UDim2.fromOffset(68, 30)
		enterButton.BackgroundColor3 = colors.accent
		enterButton.BorderSizePixel = 0
		enterButton.AutoButtonColor = false
		enterButton.Font = Enum.Font.GothamSemibold
		enterButton.Text = "Entrar"
		enterButton.TextColor3 = colors.text
		enterButton.TextSize = 11
		enterButton.Parent = row
		makeCorner(enterButton, 5)

		enterButton.MouseButton1Click:Connect(function()
			if jobId == "" then return end
			local enterJob = ZN4XLaunchEnvironment.zn4xenterjob
			if type(enterJob) ~= "function" then
				exploitNotice("API de servidores indisponivel")
				return
			end
			enterButton.Text = "Entrando..."
			local ok, entered, enterError = pcall(enterJob, jobId, {
				autoFarm = ZN4XAirsoftAutoFarmEnabled,
				server = "AirsoftFE",
			})
			if not ok or entered ~= true then
				enterButton.Text = "Entrar"
				exploitNotice("Falha ao entrar: " .. tostring(enterError or entered))
			end
		end)
	end

	local function refreshServerList()
		serverListRequestId = serverListRequestId + 1
		local requestId = serverListRequestId
		clearServerRows()
		makeServerStatus("Carregando servidores...")
		if refreshServersButton then refreshServersButton.Text = "Atualizando..." end

		task.spawn(function()
			local listServers = ZN4XLaunchEnvironment.zn4xservers
			if type(listServers) ~= "function" then
				if requestId == serverListRequestId and othersBody.Parent then
					clearServerRows()
					makeServerStatus("API de servidores indisponivel")
					if refreshServersButton then refreshServersButton.Text = "Atualizar Lista" end
				end
				return
			end

			local ok, servers, fetchError = pcall(listServers, "Desc", 2, game.PlaceId)
			if requestId ~= serverListRequestId or not othersBody.Parent then return end
			clearServerRows()
			if refreshServersButton then refreshServersButton.Text = "Atualizar Lista" end
			if not ok or type(servers) ~= "table" then
				makeServerStatus("Falha: " .. tostring(fetchError or servers))
				return
			end

			table.sort(servers, function(left, right)
				return (tonumber(left.playing) or 0) > (tonumber(right.playing) or 0)
			end)
			if #servers == 0 then
				makeServerStatus("Nenhum servidor disponivel")
				return
			end
			for index = 1, math.min(#servers, 30) do
				makeServerRow(servers[index], index)
			end
		end)
	end

	refreshServersButton = makeActionButton(othersBody, "Atualizar Lista", refreshServerList)
	refreshServersButton.Parent:SetAttribute("ZN4XServerListItem", false)
	refreshServerList()
end

local function makePlayersPage()
	local topHeight = 350
	local bottomY = topHeight + 16
	...
	local function makePlayerOption(parent, labelText, onClick, getActive, getBlocked)
		local row = Instance.new("Frame")
		row.Name = labelText:gsub("%s+", "") .. "Row"
		row.BackgroundTransparency = 1
		row.Size = UDim2.new(1, 0, 0, 28)
		...
		local function refreshButton()
			local blocked = getBlocked and getBlocked() or false
			...
			button.TextColor3 = blocked and colors.faint or (getActive and getActive() and colors.accent or colors.text)
		end

		button.MouseEnter:Connect(function()
			if getBlocked and getBlocked() then refreshButton() return end
	...
	makePlayerOption(functionsBody, "Matar Player", function()
		local selectedPlayer = getSelectedListPlayer()
		if selectedPlayer then ZN4XAirsoftStartKill(selectedPlayer, false) end
	end, nil, function()
		local selectedPlayer = getSelectedListPlayer()
		return not selectedPlayer or ZN4XAirsoftSameTeam(selectedPlayer)
	end)
	makeCompactSelector(functionsBody, "Modo", { "V1", "V2" }, function()
		return ZN4XAirsoftKillMode
	end, function(mode)
		ZN4XAirsoftKillMode = mode
	end, 52)
	makeFunctionSeparator(functionsBody)
	setFlingLoopSwitch = makeSwitch(functionsBody, "Matar Player Loop", flingLoopRunning, function(enabled)
		local selectedPlayer = getSelectedListPlayer()
		if enabled and (not selectedPlayer or ZN4XAirsoftSameTeam(selectedPlayer)) then
			notify("ZN4X Players", "Selecione um player")
			return false
		end
		flingLoopRunning = enabled
		if not enabled then
			killLoopTargetUserId = nil
			ZN4XAirsoftKillRequestId = ZN4XAirsoftKillRequestId + 1
			return
		end
		ZN4XAirsoftSetKillAllLoop(false)
		killLoopTargetUserId = selectedPlayer.UserId

		task.spawn(function()
			local observedTargetCharacter = selectedPlayer.Character
			local spawnReadyAt = ZN4XAirsoftHasSpawnProtection(observedTargetCharacter) and (os.clock() + 4) or 0
			while flingLoopRunning and not ZN4XAirsoftKillAllLoopEnabled and gui.Parent and functionsBody.Parent do
				local currentTarget = killLoopTargetUserId and Players:GetPlayerByUserId(killLoopTargetUserId) or nil
				if not currentTarget or ZN4XAirsoftSameTeam(currentTarget) then break end
				local targetCharacter = currentTarget.Character
				local targetHumanoid = targetCharacter and targetCharacter:FindFirstChildOfClass("Humanoid")
				if targetCharacter and targetCharacter ~= observedTargetCharacter then
					observedTargetCharacter = targetCharacter
					spawnReadyAt = os.clock() + 4
				end
				local spawnProtected = ZN4XAirsoftHasSpawnProtection(targetCharacter)
				local canAttack = os.clock() >= spawnReadyAt and not spawnProtected
				if targetHumanoid and targetHumanoid.Health > 0 and canAttack and not ZN4XAirsoftKillBusy then
					ZN4XAirsoftStartKill(currentTarget, true)
					repeat task.wait(0.05) until not ZN4XAirsoftKillBusy or not flingLoopRunning
					if flingLoopRunning and (not targetHumanoid.Parent or targetHumanoid.Health <= 0) then
						repeat
							task.wait(0.1)
							currentTarget = killLoopTargetUserId and Players:GetPlayerByUserId(killLoopTargetUserId) or nil
							local newCharacter = currentTarget and currentTarget.Character
							local newHumanoid = newCharacter and newCharacter:FindFirstChildOfClass("Humanoid")
							if newCharacter ~= targetCharacter and newHumanoid and newHumanoid.Health > 0 then break end
						until not flingLoopRunning or not currentTarget or not gui.Parent or not functionsBody.Parent
					end
				end
				task.wait(0.25)
			end
	...
	local function addPlayerRow(otherPlayer, distance)
		local isSelected = selectedPlayerUserId == otherPlayer.UserId
		local isSameTeam = ZN4XAirsoftSameTeam(otherPlayer)
		local teammateNameColor = Color3.fromRGB(139, 190, 148)
		local enemyNameColor = Color3.fromRGB(205, 135, 142)
		local playerNameColor = isSameTeam and teammateNameColor or enemyNameColor
		...
		local nameLabel = makeText(row, rowText, 13, playerNameColor, Enum.Font.GothamSemibold)
		...
		row.MouseEnter:Connect(function()
			if selectedPlayerUserId ~= otherPlayer.UserId then
				...
				nameLabel.TextColor3 = playerNameColor
			end
		end)
		...
		for _, otherPlayer in ipairs(Players:GetPlayers()) do
			...
			if query == "" or combinedName:find(query, 1, true) then
				table.insert(entries, {
					...
					sameTeam = ZN4XAirsoftSameTeam(otherPlayer),
				})
			end
		end

		table.sort(entries, function(left, right)
			if left.sameTeam ~= right.sameTeam then
				return not left.sameTeam
			end
...
function ZN4XRuntimeCleanup()
	...
	pcall(function() RunService:UnbindFromRenderStep("ZN4X_AutoFarmFreeMouse") end)
...
function ZN4XAirsoftDesinjetarMenu()
	ZN4XRuntimeCleanup(true)
end

function ZN4XAirsoftMakeDesinjetarButton()
	local button = Instance.new("TextButton")
	disableAutoLocalize(button)
	...
	button.MouseButton1Click:Connect(ZN4XAirsoftDesinjetarMenu)
end
...
function makeConfigPage()
	...
	local cityBody = makeSectionPanel(content, "Servidor Atual", UDim2.new(0.5, 8, 0, 162), UDim2.new(0.5, -8, 0, 206))
	local userBody = makeSectionPanel(content, "Suas Informacoes", UDim2.new(0.5, 8, 0, 384), UDim2.new(0.5, -8, 0, 206))

	makeActionButton(menuBody, "Desinjetar Menu", ZN4XAirsoftDesinjetarMenu)
...
function ZN4XAirsoftMakeNavGroup(text, layoutOrder)
...
function ZN4XAirsoftMakeNavButton(category, layoutOrder)
	local button = Instance.new("TextButton")
	disableAutoLocalize(button)
...
ZN4XAirsoftMakeNavGroup("Jogador Local", 1)
ZN4XAirsoftMakeNavButton("Jogador", 2)
ZN4XAirsoftMakeNavButton("Mira", 3)
ZN4XAirsoftMakeNavButton("Visuais", 4)

ZN4XAirsoftMakeNavGroup("Online", 5)
ZN4XAirsoftMakeNavButton("Players", 6)

ZN4XAirsoftMakeNavGroup("Outros", 7)
ZN4XAirsoftMakeNavButton("Exploit", 8)
ZN4XAirsoftMakeNavButton("Config", 9)

function ZN4XAirsoftSetMenuVisible(visible)
	menuOpen = visible
	ZN4XUpdateBlockInput()
...
connect(UserInputService.InputBegan, function(input, processed)
	...
	if input.KeyCode == Enum.KeyCode.F3 and ZN4XAirsoftAutoFarmEnabled then
		ZN4XAirsoftSetAutoFarm(false)
		if selectedCategory == "Exploit" then renderCategory("Exploit") end
		return
	end

	if input.KeyCode == ZN4XMenuBind then
		ZN4XAirsoftSetMenuVisible(not menuOpen)
		return
	end
...
ZN4XBootActive = false
renderCategory("Jogador")
ZN4XAirsoftSetMenuVisible(true)
updateCustomCursor()
if ZN4XPendingAutoFarm then
	task.spawn(function()
		local readyDeadline = os.clock() + 15
		repeat task.wait(0.2)
		until getRootPart() or os.clock() >= readyDeadline or not gui.Parent
		if gui.Parent and type(ZN4XAirsoftSetAutoFarm) == "function" then
			ZN4XAirsoftSetAutoFarm(true)
		end
	end)
end
if not modoteste then
