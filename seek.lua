...
if not modoteste then
	...
	if not validSession then
		warn("ZN4X: abra o menu pelo loginzn4x.lua.")
		return
	end
...
-- Desenvolvimento: Lista2 abre direto e nao depende do loader/API.
if modoteste then
	ZN4XExploitSelectedList = "Seek"
	ZN4XSkipBoot = true
else
	local selectedList = ZN4XValidatedLoaderSession and ZN4XValidatedLoaderSession.Server or ZN4XExploitSelectedList
	ZN4XExploitSelectedList = selectedList == "Lista" and "M" or (selectedList or "M")
	ZN4XSkipBoot = true
end
		...
		if otherPlayer ~= player then
			...
			if humanoid and humanoid.Health > 0 and rootPart then
				...
				if rootOnScreen and distance <= distanceLimit then
					...
					if ZN4XExploitSelectedList == "M" and otherPlayer.UserId == ZN4XDetectedMurderUserId then
						roleColor = Color3.fromRGB(235, 75, 95)
					elseif ZN4XExploitSelectedList == "M" and otherPlayer.UserId == ZN4XDetectedPolicialUserId then
						roleColor = Color3.fromRGB(80, 235, 120)
					end
...
function makeExploitPage()
	...
	if ZN4XExploitSelectedList == "Lista3" then
		makeActionButton(exploitsBody, "Funcao Lista3", function()
			exploitNotice("Lista3")
		end)

		makeActionButton(exploitsBody, "Outra Funcao Lista3", function()
			exploitNotice("Outra funcao da Lista3")
		end)
	elseif ZN4XExploitSelectedList == "M" then
		local policialButton = makeActionButton(exploitsBody, "Matar Policial", function()
			runZN4XExploitAction("kill", "Policial")
		end)
		local setAutoPolicialSwitch, autoPolicialSwitchButton = makeSwitch(exploitsBody, "Matar Policial Auto", ZN4XAutoKillPolicialEnabled, function(enabled)
			if ZN4XForcedGunPickupBusy then
				notify("ZN4X", "Erro Aguarde")
				return false
			end
			ZN4XAutoKillPolicialEnabled = enabled
			ZN4XAutoKillPolicialLastUserId = nil
			ZN4XAutoKillPolicialNext = 0
		end)
		...
		local murderButton = makeActionButton(exploitsBody, "Matar Assassino", function()
			runZN4XExploitAction("kill", "Murder")
		end)
		local setAutoMurderSwitch, autoMurderSwitchButton = makeSwitch(exploitsBody, "Matar Murder Auto", ZN4XAutoKillMurderEnabled, function(enabled)
			if ZN4XForcedGunPickupBusy then
				notify("ZN4X", "Erro Aguarde")
				return false
			end
			ZN4XAutoKillMurderEnabled = enabled
			ZN4XAutoKillMurderLastUserId = nil
			ZN4XAutoKillMurderNext = 0
		end)
		...
		local setAntiMurderSwitch, antiMurderSwitchButton = makeSwitch(exploitsBody, "Anti Murder", ZN4XAntiMurderEnabled, function(enabled)
			if ZN4XForcedGunPickupBusy then
				notify("ZN4X", "Erro Aguarde")
				return false
			end
			ZN4XAntiMurderEnabled = enabled
			ZN4XAntiMurderNext = 0
		end)

		local setAntiPolicialSwitch, antiPolicialSwitchButton = makeSwitch(exploitsBody, "Anti Policial", ZN4XAntiPolicialEnabled, function(enabled)
			if ZN4XForcedGunPickupBusy then
				notify("ZN4X", "Erro Aguarde")
				return false
			end
			ZN4XAntiPolicialEnabled = enabled
			ZN4XAntiPolicialNext = 0
		end)

		makeFunctionSeparator(exploitsBody)

		local setPegarArmaSwitch, pegarArmaSwitchButton = makeSwitch(exploitsBody, "Pegar Arma", ZN4XAutoGetGunEnabled, function(enabled)
			if ZN4XForcedGunPickupBusy then
				notify("ZN4X", "Erro Aguarde")
				return false
			end
			ZN4XAutoGetGunEnabled = enabled
			resetZN4XAutoGetGunState()
			if enabled then
				local policial = findZN4XBackpackRoleHolder(ZN4XPolicialToolNames, player)
				if policial then setZN4XObservedPolicial(policial) end
			end
		end)

		makeFunctionSeparator(exploitsBody)

		local forcedGunButton = makeActionButton(exploitsBody, "Pegar Arma Forcado", function()
			if ZN4XForcedGunPickupBusy or ZN4XShootMurderBusy then
				notify("ZN4X", "Erro Aguarde terminar")
				return
			end
			local policial = ZN4XDetectedPolicialUserId and Players:GetPlayerByUserId(ZN4XDetectedPolicialUserId) or nil
			local policialHumanoid = policial and policial.Character and policial.Character:FindFirstChildOfClass("Humanoid")
			if not policial or not policialHumanoid or policialHumanoid.Health <= 0 then
				notify("ZN4X", "Erro ao encontrar o policial")
				return
			end
			notify("ZN4X", "Tentando Puxar Arma")
			runZN4XForcedGunPickup()
		end)
		forcedGunButton.Active = true
		forcedGunButton.TextColor3 = colors.faint
		forcedGunButton.BackgroundColor3 = Color3.fromRGB(13, 15, 23)

		makeFunctionSeparator(exploitsBody)

		local shootMurderButton = makeActionButton(exploitsBody, "Atirar no Assassino", function()
			runZN4XShootMurder()
		end)
		makeCompactSelector(exploitsBody, "Modo Atirar", { "V1", "V2" }, function()
			return ZN4XShootMurderMode
		end, function(mode)
			if ZN4XShootMurderBusy then
				notify("ZN4X", "Erro Aguarde")
				return
			end
			ZN4XShootMurderMode = mode
		end, 62)

		local nextRoleLabelRefresh = 0
		connectPage(RunService.RenderStepped, function()
			if os.clock() < nextRoleLabelRefresh then return end
			nextRoleLabelRefresh = os.clock() + 0.25

			local policial = ZN4XDetectedPolicialUserId and Players:GetPlayerByUserId(ZN4XDetectedPolicialUserId) or nil
			local murder = ZN4XDetectedMurderUserId and Players:GetPlayerByUserId(ZN4XDetectedMurderUserId) or nil
			if policialButton and policialButton.Parent then
				policialButton.Text = policial and ("Matar Policial | " .. policial.Name) or "Matar Policial"
			end
			if murderButton and murderButton.Parent then
				murderButton.Text = murder and ("Matar Murder | " .. murder.Name) or "Matar Murder"
			end
			setAutoPolicialSwitch(ZN4XAutoKillPolicialEnabled)
			setAutoMurderSwitch(ZN4XAutoKillMurderEnabled)
			setAntiMurderSwitch(ZN4XAntiMurderEnabled)
			setAntiPolicialSwitch(ZN4XAntiPolicialEnabled)
			setPegarArmaSwitch(ZN4XAutoGetGunEnabled)
			local controlsUnlocked = not ZN4XForcedGunPickupBusy
			for _, switchButton in ipairs({
				autoPolicialSwitchButton,
				autoMurderSwitchButton,
				antiMurderSwitchButton,
				antiPolicialSwitchButton,
				pegarArmaSwitchButton,
			}) do
				switchButton.Active = true
				switchButton.TextTransparency = controlsUnlocked and 0 or 0.55
			end
			if forcedGunButton and forcedGunButton.Parent then
				local unlocked = policial ~= nil and not ZN4XForcedGunPickupBusy
				forcedGunButton.Active = true
				forcedGunButton.TextColor3 = unlocked and colors.text or colors.faint
				forcedGunButton.BackgroundColor3 = unlocked and Color3.fromRGB(24, 29, 42) or Color3.fromRGB(13, 15, 23)
			end
			if shootMurderButton and shootMurderButton.Parent then
				local shootUnlocked = murder ~= nil and findZN4XShootTool() ~= nil and not ZN4XShootMurderBusy
				shootMurderButton.Active = true
				shootMurderButton.TextColor3 = shootUnlocked and colors.text or colors.faint
				shootMurderButton.BackgroundColor3 = shootUnlocked and Color3.fromRGB(24, 29, 42) or Color3.fromRGB(13, 15, 23)
			end
		end)
	elseif ZN4XExploitSelectedList ~= "Seek" then
		makeActionButton(exploitsBody, "Funcao Lista", function()
			exploitNotice("Lista")
		end)

		makeActionButton(exploitsBody, "Outra Funcao Lista", function()
			exploitNotice("Outra funcao da Lista")
		end)
	end
...
function showZN4XBoot()
	...
	local title = makeText(panel, "CARREGANDO ZN4X", 21, colors.text, Enum.Font.GothamBold)
	title.Position = UDim2.fromOffset(28, 34)
	...
	barBack.Position = UDim2.fromOffset(38, 118)
	barBack.Size = UDim2.new(1, -76, 0, 10)
	barBack.BackgroundColor3 = Color3.fromRGB(31, 38, 54)
	barBack.BorderSizePixel = 0
	barBack.ZIndex = 2002
	barBack.Parent = panel
	makeCorner(barBack, 5)
	...
	local subtitle = makeText(panel, "Preparando menu", 12, colors.muted, Enum.Font.GothamMedium)
	subtitle.Position = UDim2.fromOffset(28, 142)
...
connect(RunService.RenderStepped, function(deltaTime)
	...
	if not ZN4XBootActive and ZN4XExploitSelectedList == "M" and os.clock() >= ZN4XRoleScanNext then
		ZN4XRoleScanNext = os.clock() + 1
		scanZN4XRoles(true)
	end
